 ...
 * @author ruoyi
 */
public class GatherExcelUtil<T> {
    private static final Logger log = LoggerFactory.getLogger(GatherExcelUtil.class);
    ...
    public GatherExcelUtil(Class<T> clazz) {
        this.clazz = clazz;
    }
     ...
     * @return 转换后集合
     */
    public List<T> importExcel(InputStream is, int titleNo, int dataNo) throws Exception {
        return importExcel(StringUtils.EMPTY, is, titleNo, dataNo);
    }

    public AjaxResult exportGatherExcel(List<T> list, String sheetName) {
     ...
     * @return 转换后集合
     */
    public List<T> importExcel(String sheetName, InputStream is, int titleNo, int dataNo) throws Exception {
        ...
        if (rows > 0) {
            ...
            // 获取表头
            Row heard = sheet.getRow(titleNo); // 0
